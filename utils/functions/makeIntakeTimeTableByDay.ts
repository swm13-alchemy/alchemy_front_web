...
export function makeIntakeTimeTableByDay(intakePillList: IntakeManagementType[], intakeTimeTableData: TimeTableByDayType, setIntakeTimeTableData: (intakeTimeTableData: TimeTableByDayType) => void) {
  ...
  if (arrayIsNotEmpty(intakePillList)) {
    ...
    for (const intakePill of intakePillList) {
      ...
      const addDataToTimeKey = function (day: Days) {
        for (const intakeTime of intakePill.intakeTime) {
          // 시간 값을 '0:0' 형태로 변환
          const intakeTimeStr: string = intakeTime.join(':')

          // 현재 해당 요일에 등록된 스케줄이 아예 없거나(null)
          // 해당 시간 값(intakeTimeStr)을 key로 가지고 있지 않다면,
          if (intakeTimeTableData[day] === null ||
            !Object.prototype.hasOwnProperty.call(intakeTimeTableData[day], intakeTimeStr)) { // !intakeTimeTableData.Sun.hasOwnProperty(intakeTimeStr) / es-lint로 인해 수정
            // 해당 요일의 해당 시간 값을 key로 데이터를 넣음
            setIntakeTimeTableData({...intakeTimeTableData, [day]: {[intakeTimeStr]: [timeTableData]}})
          } else {  // 해당 시간 값(intakeTimeStr)을 key로 가지고 있는 경우,
            // TODO: @ts-ignore 치우고 로직 구현 (앞에 if문에서 null 처리해줘서 오류날게 없는데 계속 남)
            // @ts-ignore
            const curScheduleData: TimeTableType[] = intakeTimeTableData[day][intakeTimeStr]
